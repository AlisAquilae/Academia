---
fileClass: Base, Lecture Notes
title: 
Parents: 
Draft: 
Status: 
Related Notes: 
Related Terminology: 
Related Courses: 
---
# Today
River bedforms

Interaction between hydrodynamics and sediment transport. 

- Bedform definitions and importance
- Bedform classification
- Equilibrium dimensions
- Flow resistance
- Stage-discharge
- State of the art and current understanding. 

---
# Dunes definition
Dunes are also present underwater, sub-aqueous dunes. 

A bedform is a feature that
- Develops on the interface of a flow and a movable bed
- And is the result of bed material being moved by this flow

We will look at unidirectional bedforms (e.g. river flows). But you could also think of estuary or sea bedforms, which are affected by multiple directions. 

Implications of bedform dynamics
- Determine flow resistance, influence flood management
- Influence river bed stability. 
- Influence sediment transport. 
- Influence navigability

---
# Bedform classification
Lower-stage plane bed
- Flow velocity is not sufficient to form bedforms. Little bedload transport. 

Ripples:
- Slight bedload transport
- Evenly spaced, cm-scale, fine sand, subcritical flow
- Develop in the laminar/viscous sublayer
- Hydraulically smooth regime (grainsizes smaller than depth of laminar layer)
- No interaction with water surface
- Dimensions scale with grain size
- Formed solely by bed load

Dunes
- More bedload transport
- Larger, 2D and 3D, sand and gravel (coarser than with ripples), subcritical flow
- Migrate downstream with a pronounced slip face (leeway side)
- May generate flow separation (hydraulic roughness)
- Water surface is affected, out of face with the river bed
- Scale with water depth (contrary to ripples)
- Formed by bed load and suspended load

Upper-stage plane bed
- More sediment in suspension, little dunes (dunes are filled in, turbulence is dampened)
- Unstable regime (easy to form dunes, but can also easily remove them again)
- Supercritical flow
- Suspended sediment

Antidunes
- Supercritical flow
- Cause water surface undulation (in phase with topography)
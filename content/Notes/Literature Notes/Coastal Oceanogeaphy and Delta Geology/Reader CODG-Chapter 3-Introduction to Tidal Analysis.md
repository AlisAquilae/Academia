...
$$
\tan(\alpha) = \frac{\partial \zeta}{\partial x} \tag{3.17}
$$

so that

$$
g\frac{\partial \zeta}{\partial x} + \frac{\partial \Omega_p}{\partial x} = 0 \tag{3.18}
$$

or

$$
\frac{\partial}{\partial x} (g\zeta + \Omega_p) = 0 \tag{3.19}
$$
...
$$
\zeta(\varphi_p, t) = r \left( \frac{m_l}{m_e} \right) \left( \frac{r}{R_l} \right)^3 \left( C_0(t)(\frac{3}{2} \sin^2 (\varphi_p) - \frac{1}{2}) + C_1 (t) \sin(2\varphi_p) + C_2 (t) \cos^2 (\varphi_p) \right) \tag{3.22}
$$
...
The aim of this chapter is to describe the tide at a fixed point of observation with a finite set of time-stable significant parameters. The form of the description is suitable for prediction of tides and related to the physical processes of tide generation. Harmonic tidal analysis is a basic method to do this. It describes the tide as the sum of a set of sinusoids at specific frequencies based on astronomical cycles described above. 

>[!Info]
>This section is largely based on chapter 4 of Pugh (1996) and Pawlowicz et al. (2002)


In coastal environments, tides are often the largest source of variability. Power spectra of time series show high energy at small frequencies and a decline at higher frequencies. Analysis of these time series often requires a separation of the tidal signal from sub- or super-tidal variations. In other words, the astronomic variation should be filtered from the non-astronomic variation. The last can be generated by wind effects, river discharge or other non-linear effects. The implicit assumption in harmonic analysis is that tidal properties are stationary, independent of the non-astronomical forcing functions

## 3.2.1 Classic Harmonic Analysis
In classic harmonic analysis, the tidal harmonics are presented as a sum of a finite set of sinusoids. This was already shown in equation 3.22 of the equilibrium tide. This is a Fourier series of the tidal signal and is seen as the basis of harmonic analysis. The sinusoids have specific frequencies which are related to astronomical parameters. 

Generally, a harmonic term has the following form:

$$
\zeta_n(t) = H_n \cos(\omega_n t - g_n) \tag{3.24}
$$

Where,
- $\zeta_n(t)$ is the free water level;
- $H_n$ is the amplitude of the harmonic;
- $g_n$ is the phase lag with respect to Equilibrium Tide at Greenwhich; and
- $\omega_n$ is the frequency

>[!Note]
>Greenwhich (England) is the zero point of the time zones on earth and the time reference of tidal propagation. 
